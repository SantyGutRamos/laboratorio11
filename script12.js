function iniciarCarritoConPresupuesto() {
    let presupuestoInput = prompt("Ingresa tu presupuesto total:");
    let presupuesto = parseFloat(presupuestoInput);
    ...
    while (true) {
        let precioInput = prompt("Presupuesto: $" + presupuesto.toFixed(2) +
                               "\nTotal actual: $" + totalParcial.toFixed(2) +
 ...
 if (isNaN(precio) || precio < 0) {
        ...
        if ((totalParcial + precio) > presupuesto) {
            alert("No puedes agregar este producto. Excede tu presupuesto.");
            continue;
        } 
        totalParcial += precio;
    }
